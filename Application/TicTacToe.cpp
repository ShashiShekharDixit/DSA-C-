...
//Checks if three items are the same, and makes sure they're not 0's.
#define numberToLetter(x) ((x>0) ? (x == 1) ? 'x' : '0' : ' ')
//Takes the number and turns it into the letter or space,
int getWinner(int board[9]) 
{
     //Finds winner of game, if there is no winner, return 0.
     int winner = 0;
     for (int x = 0; x < 3; x++)
     {
     ...
     int exceptional(int board[9])
     {
      //Finds board that are exceptions to how the algorithm works.
      int cases[2][9] = {{1,0,0,0,2,0,0,0,1}, {0,1,0,1,2,0,0,0,0}}; //Boards that don't worl eith algorithm.
      ...
      for(int caseIndex = 0; caseIndex < 2; caseIndex++)
      {
            for (int rotation = 0; rotation < 4; rotation++)
            {
                ...
                int match = 0;
                //Rotates board so it works with differnt versions of the same board.
...
int main()
      {
         ...
         {   //Player X decides what move they'll do.
            do {
                 ...
                 else if (exceptional(board)> -1)board[exceptional(board)] = 2; //Exception boards;
